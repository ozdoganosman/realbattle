...
export function createRenderer(sim, mapCanvas, fxCanvas) {
  ...
  function paintLabels() {
    ...
    for (const nat of sim.nations) {
      ...
      // Adın altında garnizondaki asker — herkesinki görünsün. Ulus renkleri
      // koyudan açığa her tonda olduğu için sayı doğrudan haritaya yazılınca
      // okunmuyordu; parşömen bir hapın üstüne basılıyor ki her zemin üstünde
      // aynı kontrastta dursun.
      const ns = clamp(size * 0.7, 11, 19);
      mctx.font = `bold ${ns}px Georgia, serif`;
      const borclu = nat.pool < 0;
      const yazi = borclu ? `−${kisa(-nat.pool)}` : kisa(nat.pool);
      const ty = y + size * 0.86;
      const tw = mctx.measureText(yazi).width;
      const px = ns * 0.42, ph = ns * 1.24, r = ph / 2;
      mctx.beginPath();
      const bx = x - tw / 2 - px, bw = tw + px * 2;
      if (mctx.roundRect) mctx.roundRect(bx, ty - ph / 2, bw, ph, r);
      else mctx.rect(bx, ty - ph / 2, bw, ph);
      mctx.fillStyle = 'rgba(250,244,225,0.92)';
      mctx.fill();
      mctx.strokeStyle = 'rgba(44,28,12,0.55)';
      mctx.lineWidth = 1;
      mctx.stroke();
      mctx.fillStyle = borclu ? 'rgb(168,38,22)' : 'rgb(46,30,12)';
      mctx.fillText(yazi, x, ty);
    }
  }
